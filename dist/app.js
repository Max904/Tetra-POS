  ...
  switch (action.type) {
    ...
    case "ADD_TO_ORDER": {
      ...
      const next = withOrder(state, action.orderId, (o) => {
        ...
        const items = idx >= 0 ? o.items.map((it, i) => i === idx ? { ...it, qty: it.qty + 1 } : it) : [...o.items, { menuId: action.menuId, name: action.name, price: action.price, qty: 1, note: "", seat, done: false, ready: false, sale: false }];
        return { ...o, items };
      });
      return {
        ...next,
  ...
  switch (action.type) {
    ...
    case "SET_ITEMS_SALE": {
      ...
      const results = await Promise.all(
        targets.map((item) => {
          ...
          let q = supabase.from("order_items").update({ sale: !!action.sale }).eq("order_id", action.orderId).eq("menu_id", item.menuId);
    ...
    case "SET_ITEMS_READY": {
      ...
      const results = await Promise.all(
        targets.map((item) => {
          const seat = item.seat ?? null;
          let q = supabase.from("order_items").update({ ready: !!action.ready }).eq("order_id", action.orderId).eq("menu_id", item.menuId);
          return seat === null ? q.is("seat", null) : q.eq("seat", seat);
        })
      );
      const failed = results.find((r) => r.error);
      ...
      return /* @__PURE__ */ jsxDEV("article", { className: `ticket-card ${o.barStatus} ${red ? "over" : ""}`, children: [
        ...
        /* @__PURE__ */ jsxDEV("div", { className: "k-groups", children: groupByCategory2(o.items, state.menu, state.categories).map((group) => /* @__PURE__ */ jsxDEV("div", { className: "k-cat-group", children: [
          ...
          /* @__PURE__ */ jsxDEV("ul", { className: "k-items", children: group.items.map((it, i) => /* @__PURE__ */ jsxDEV("li", {
            ...
            onClick: () => dispatch({ type: "TOGGLE_ITEM_DONE", orderId: o.id, index: it.index, done: !it.done }),
            children: [
              jsxDEV("span", { className: "k-check", children: it.done && jsxDEV(Check2, { size: 14, strokeWidth: 3 }) }),
              /* @__PURE__ */ jsxDEV("span", { className: "k-qty", children: [
                it.qty,
                "\xD7"
              ] }, void 0, true, {}, this),
